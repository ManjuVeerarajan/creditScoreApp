...
//public static void main(String[] args) {
	...
	public static void main(String[] args) {
		try {
			...
			String ccrisindividualinfo = StringUtils.substringBetween(xmlResponse, "<ccris_individual_info>",
	...
	// the XSL FO file
	/*
	 * File xsltFile = new File(RESOURCES_DIR + "//NewStylesheet.xsl"); // the XML
	 * file which provides the input StreamSource xmlSource = new StreamSource(new
	 * File(RESOURCES_DIR + "//data.xml")); // create an instance of fop factory
	 * FopFactory fopFactory = FopFactory.newInstance(new File(".").toURI()); // a
	 * user agent is needed for transformation FOUserAgent foUserAgent =
	 * fopFactory.newFOUserAgent(); // Setup output OutputStream out; out = new
	 * java.io.FileOutputStream(OUTPUT_DIR + "//output.pdf");
	 * 
	 * 
	 * try { // Construct fop with desired output format Fop fop =
	 * fopFactory.newFop(MimeConstants.MIME_PDF, foUserAgent, out);
	 * 
	 * // Setup XSLT TransformerFactory factory = TransformerFactory.newInstance();
	 * Transformer transformer = factory.newTransformer(new StreamSource(xsltFile));
	 * 
	 * // Resulting SAX events (the generated FO) must be piped through to // FOP
	 * Result res = new SAXResult(fop.getDefaultHandler());
	 * 
	 * // Start XSLT transformation and FOP processing // That's where the XML is
	 * first transformed to XSL-FO and then // PDF is created
	 * transformer.transform(xmlSource, res); } finally { out.close(); } }
	 */
	 ...
	 * FopFactory fopFactory = FopFactory.newInstance(new File(".").toURI()); // a
	 * user agent is needed for transformation FOUserAgent foUserAgent =
	 * fopFactory.newFOUserAgent(); // Setup output String filename=OUTPUT_DIR
	 * +"950725-14-6373"+".pdf"; System.out.println(filename); OutputStream out; out
	 ...
	 * 
	 * 
	 * } finally { out.close(); } return filename; }
	 */
	public void convertToPDF() throws Exception {
		 ...
		 */
		/*
		 * StreamSource xmlSource = new StreamSource(new File(path)); // create an
		 * instance of fop factory FopFactory fopFactory = FopFactory.newInstance(new
		 * File(".").toURI()); // a user agent is needed for transformation FOUserAgent
		 * foUserAgent = fopFactory.newFOUserAgent(); // Setup output String
		 * filename=OUTPUT_DIR+"999999"+".pdf";
		 * System.out.println("========="+filename); System.out.println(filename);
		 * OutputStream out; out = new java.io.FileOutputStream(filename);
		 * 
		 * // System.out.println("File path: "+directoryPath.getAbsolutePath());
		 * 
		 * 
		 ...
		 * // Start XSLT transformation and FOP processing // That's where the XML is
		 * first transformed to XSL-FO and then // PDF is created
		 * transformer.transform(xmlSource, res); // fw.flush(); } finally {
		 * //Files.delete(Paths.get(path)); out.close(); // fos.close(); // } return
		 * filename;
		 * 
		 * }
		 */
